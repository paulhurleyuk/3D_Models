...
//	Spool dimensions
srimd = 50;		//	Spool rim diameter
srr = srimd/2;	//	Spool rim radius
srth = 2;		//	Spool rim thickness
srh= 10;         // spool rim height
sbh= 5;         // spool bottom rim height
...
function main() {
  return [
	  ...
	  difference(
	    difference(
		union(
		    translate([0,0,sbh+ssh], cylinder({r1: ssr,r2: srr,h: srh,fn: fn})),
		    translate([0,0,sbh], cylinder({r: ssr, h: ssh, fn: fn})),
		...
		cube({size: [m3w,m3af,srh+(ssh/2)+(m3ac/2)+2], center: false})
		),
